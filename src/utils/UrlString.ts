/*
 * adapted from Microsof msal library.

 */

import { StringUtils } from "./StringUtils";
import { AADAuthorityConstants, Constants } from "./Constants";
...
export class UrlString {
  ...
  constructor(url: string) {
    ...
    if (StringUtils.isEmpty(this._urlString)) {
      ...
      throw console.error("url is empty! => ", url);
    }
  ...
  validateAsUri(): void {
    ...
    try {
      ...
      throw console.error("uri validation error", e);
    }

    // Throw error if URI or path segments are not parseable.
    if (!components.HostNameAndPort || !components.PathSegments) {
      throw console.error(
        `validation error, Given url string: ${this.urlString}`
      );
    }
    ...
    if (
      ...
      throw console.error("insecure uri error (https?)!", this.urlString);
    }
  }
  ...
  getUrlComponents(): IUri {
    ...
    if (!match) {
      throw console.error(
        `get component error, Given url string: ${this.urlString}`
      );
    }
  ...
  static getDomainFromUrl(url: string): string {
    ...
    if (!match) {
      throw console.error(`url get domain error, Given url string: ${url}`);
    }
